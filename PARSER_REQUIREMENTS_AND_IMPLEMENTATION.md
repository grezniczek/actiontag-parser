...
zero-count conditions remain uncluttered. Selecting a nonzero count activates
...
the matching ACE columns. A nested enabled `@IF` case confirms its link selects
that direct condition rather than its outer container, including Edit activation
and dialog-listener cleanup.
...
Settings' Confirmation Email Subject readonly focus/click launchers also
exercise the shared pending-load marker guard: each pair of rendered events
produces one workspace after dependencies resolve while retaining its
established click cancellation and focus return. The Confirmation Email Subject
must retain its one-line readonly control and return focus to the confirmation
enable checkbox. The Data Entry Trigger URL and End Survey Redirect URL's
existing blur validators must continue to defer while that marker is present,
...
The selected-row path with no available label cell has the same no-buffer,
...
marker so a following Field Note authoring gesture can retry. A double-click
while its workspace is already visible refocuses the existing ACE editor
without loading dependencies or creating another dialog.
...
a following focus can retry from the same source.
