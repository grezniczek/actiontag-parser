...
- Field Annotation, Quick-modify fields' custom Action Tags source,
  ...
  While visible, another delegated click focuses its existing ACE editor
...
- Survey Settings now provides the same explicit action beside its existing
  ...
  workspace and its confirmation-enable focus return, while the redirect URL
  preserves its cancellable click, focus-selected URL mode, and marker-aware
  URL validation until workspace closure. The auto-continue condition uses the
  logic workspace in the same way. Its browser coverage preserves its
  checkbox-controlled disabled/dimmed state, shared pending-load guard once
  enabled, and existing logic validation on both workspace save and blur. These
  Survey Settings sources are piping-only: field embedding remains disabled
  there.
...
- Project Setup's Custom Record Label, Define My Events' Custom Event Label,
  ...
  Custom Record Label, Custom Event Label, and Custom Repeating Instrument
...
- Project Dashboard body text has an explicit TinyMCE source-editor action.
  ...
  shared opener guard before it is cleared on close. The direct textarea path
  retains no bridge buffer reference; its duplicates go directly through that
  shared opener marker. An unavailable source keeps the existing
